...
data modify block 0 -64 0 Items[0].components."minecraft:lodestone_tracker".target.pos set from entity @p[distance=0.1..] Pos

execute as @p[distance=0.1..] run function #bs.position:get_distance_ata {scale:1}
execute as @p[distance=0.1..] run tellraw @p[distance=..1] [{"text": "[寻人罗盘] 找到 ","color": "gray"},{"selector": "@s","color": "yellow"},{"text": " 的位置，距离 ","color": "gray"},{"score": {"name": "$position.get_distance_ata","objective": "bs.out"},"color": "yellow"},{"text": " 格","color": "gray"}]
